...
if [ -f "$PWD/dist/${asset_filename}" ]; then
  ...
  if [[ "$(docker images -q ${asset_image} 2> /dev/null)" == "" ]]; then
    ...
    docker build --build-arg "RUBY_VERSION=$ruby_version" --build-arg "RUBY_INSTALL_VERSION=${ruby_install_version:-0.8.5}" --build-arg "ASSET_VERSION=$asset_version" -t ${asset_image} -f Dockerfile.${platform} .
